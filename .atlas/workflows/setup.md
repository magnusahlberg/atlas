...
Interactive setup. Tell Atlas about yourself so it can work the way you do.
...
- Anything else that matters to you in how Atlas communicates with you?
...
Then check what is actually reachable from this session. List the tools you can see for mail search, calendar search, chat search, and sending a message. Do not guess: if you cannot see a tool for something, it is not available.
...
Ask:
- Do you have a shared canvas or standing agenda doc with your manager — for example a chat canvas you both maintain?
...
1. **Write `Reference/Personal.md`** with sections: Role, Team, Communication preferences, Working style, Connected tools, Timezone, Shared canvas. Use the information collected above. Write in plain prose, not bullet dumps — this file is read at the start of every session to calibrate tone and context. Include:
   - **Timezone**: the abbreviations and UTC offsets (e.g. "CET (UTC+1) / CEST (UTC+2)")
   - **Shared canvas**: if configured, include the person's name, canvas ID, and link. If not, omit the section.
...
3. **Write `Reference/Tools.md`** from Section 5. One row per capability: mail search, mail tagging, calendar search, chat search, send message. For each, name the tool if one is reachable, or write "not available". Keep it to capability names, not the harness-specific tool identifiers, since those differ per harness and go stale. See the existing file for the format if it is already there.

4. **Summarize** what was written and what to do next (e.g. run the `daily-plan` workflow to get started).
